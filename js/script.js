...
function allTasks(){
      ...
      //if tasks' length is 0 then pending num text content will be no,if not then  pending num  will be tasks length
...
inputField.addEventListener("keyup", (e) => {
    let inputVal = inputField.value.trim();//trim function removes space of front and back of inputed value
...
function handleStatus(e){
    ...
    checkbox.checked = checkbox.checked ? false : true; //if checked then false else true
    e.classList.toggle("pending");
    allTasks();
}
//deleting taask while we click on the delete icon.
